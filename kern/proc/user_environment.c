...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
	// [1] Allocate the stack of size KERNEL_STACK_SIZE
	// The stack grows downwards, so kmalloc returns the lowest address (the bottom)
	// which is exactly where we want to place the Guard Page.
	void* stack_va = kmalloc(KERNEL_STACK_SIZE);

	// [2] Check for allocation failure
	if (stack_va == NULL)
	{
		panic("create_user_kern_stack: failed to allocate user kernel stack");
	}

	// [3] Mark its bottom page as NOT PRESENT (GUARD page)
	// We need to find the Page Table containing this address within the user's directory
	uint32* ptr_page_table = NULL;

	// get_page_table is defined in paging_helpers.c (included via memory_manager.h)
	get_page_table(ptr_user_page_directory, (uint32)stack_va, &ptr_page_table);

	if (ptr_page_table != NULL)
	{
		// Access the entry using the PTX macro and clear the PRESENT bit
		// This ensures that if the stack overflows downwards, it hits this page and faults.
		ptr_page_table[PTX((uint32)stack_va)] &= ~(PERM_PRESENT);

		// CRITICAL: Invalidate the TLB for this address.
		// Even though we modified the table, the CPU might have cached the page as "Present"
		// when kmalloc allocated it. We must force a refresh.
		tlb_invalidate(ptr_user_page_directory, stack_va);
	}
	else
	{
		// If kmalloc returned a valid address, the page table *must* exist.
		panic("create_user_kern_stack: page table missing for allocated stack");
	}

	// [4] Return pointer to the created stack
	return stack_va;
}
