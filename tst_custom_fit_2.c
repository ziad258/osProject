...
#define numOfFFTests 3
...

	{
		...
		for (int i = 0; i < numOfFFTests; ++i)
		{
			...
			//Check returned va
			if (check_dynalloc_datastruct(tstStartVAs[i], (void*) expectedVAs[i], expectedSizes[i], 1) == 0)
			{
				is_correct = 0;
	...
	cprintf("3: Try to allocate a block with equal to the first existing free block\n\n") ;
	{
		...
		if (check_dynalloc_datastruct(tstStartVAs[numOfFFTests], expectedVA, expectedSize, 1) == 0)
		{
			is_correct = 0;
